...
const EditChapter = () => {
    ...
    const currentTimeInIST = useCurrentTimeInIST();
    ...
    useEffect(() => {
        if (Object.keys(chapters).length > 0) {
            const selectedChapter = Object.fromEntries(
            ...
            setFormData({
                ...
                storyId: storyId,
                chapterId: chapterId,
                title: selectedChapter[chapterId].ChapterTitle,
                url: selectedChapter[chapterId].ChapterUrl,
                chapterNo: selectedChapter[chapterId].ChapterNo,
                scheduledOn: selectedChapter[chapterId].ScheduledOn,
                chapterContentDoc: null,
                content: selectedChapter[chapterId].Story,
                uploadTime: currentTimeInIST,
            })
        }
    ...
    const handleFormSubmit = async () => {
        try {
            const plainText = slateRef.current ? slateRef.current.getText() : '';
            console.log(JSON.stringify({ ...formData, plainText }))

            const response = await toast.promise(
                fetch('https://us-central1-cb-story-app-27012024.cloudfunctions.net/updateChapter', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleDelete = (deleteChapterId, chapterTitle) => {
        if (deleteChapterId != chapterId) {
            ...
            confirmAlert({
                title: "Cannot Delete 🤔",
                message: `You cannot delete the chapter, you are editing`,
                buttons: [
                    {
                        label: "Oops, okay",
                    },
    ...
    return (
        <div className="page container-fluid">
            <ToastContainer />
            {loading ? <div className="loader" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)" }}></div> : <div className="row form-holder">
                            ...
                            <ul className="list-chapters" style={{ position: "relative", minHeight: "30rem" }}>
                                {Object.keys(chapters).length > 0 ? (
                                    Object.entries(chapters).map(([key, chapter], index) => (
                                        <li className={(key === chapterId) ? "list-item active" : "list-item"} style={{ padding: "0.7rem 1rem" }} key={key}>
